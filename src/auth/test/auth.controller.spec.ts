...
describe('AuthController', () => {
  ...
  describe('signin', () => {
    ...
    it('deve lançar UnauthorizedException se exceder tentativas', async () => {
      ...
      await expect(
        authController.signin(userData, res as Response, req as Request)
      ).rejects.toThrow(UnauthorizedException);

      expect(rateLimiterService.checkLoginAttempt).toHaveBeenCalledWith(
        'some-ip',
        'test@test.com'
      );
    });
  ...
  describe('me', () => {
    ...
    it('deve lançar UnauthorizedException se não houver token', async () => {
      ...
      await expect(
        authController.me(reqNoToken as Request)
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('deve retornar payload do usuário se token for válido', async () => {
      ...
      const result = await authController.me(reqWithToken as Request);
